...
export default function InternalTransaction({ accountAddress }) {
    ...
    const fetchTransactions = async () => {
      ...
      try {
        const response = await fetch(
          `https://evmapi-testnet.confluxscan.net/api?module=account&action=txlistinternal&address=${accountAddress}&page=1&offset=10&sort=asc`
        )
        const data = await response.json()
        console.log('Internal transaction data:', data)
        ...
        <div className="bg-gray-900 bg-opacity-70 backdrop-filter backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden border border-gray-700">
          {loading ? (
            ...
            <div className="text-center p-8">
              <p className="text-2xl font-bold text-gray-300">No internal transactions found</p>
              <p className="text-gray-400 mt-2">This address has no internal transactions</p>
            </div>
          ) : (
                ...
                <tbody className="bg-gray-900 divide-y divide-gray-700">
                  {transactions.map((txn, index) => (
                    <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-900'} hover:bg-gray-700 transition-colors duration-200`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        {txn.from.substring(0, 10)}...{txn.from.substring(txn.from.length - 8)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        {txn.to.substring(0, 10)}...{txn.to.substring(txn.to.length - 8)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        {(Number(txn.value) / 1e18).toFixed(4)} CFX
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a
                          href={`https://evmtestnet.confluxscan.net/tx/${txn.hash}`}
                          target="_blank"
